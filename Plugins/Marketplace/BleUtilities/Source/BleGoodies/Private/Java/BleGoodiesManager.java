...
public class BleGoodiesManager {
	...
	private java.util.Set<BluetoothDevice> mPairedDevices = mBluetoothAdapter.getBondedDevices();
	private List<String> mPairedDevicesList = new ArrayList<String>();
	...
	public void startScanForDevices(String[] uuids) {
		...
		BleGoodiesUtils.log("beyond startScanForDevices 1");

		if (mBluetoothAdapter == null) return;

		BleGoodiesUtils.log("beyond startScanForDevices 2");
		/* original
		...
		*/
		...
		{
			BleGoodiesUtils.log("beyond startScanForDevices 3");

			BleGoodiesUtils.log(String.format("beyond Device found: %s, name: %s", device, device.getName()));
			onDeviceFound(mCallbackAddr, new BleGoodiesDevice(mActivity,device));
		}
	}
	...
	public List<String> getPairedDeviceNameList() {
		for(BluetoothDevice bt : mPairedDevices)
			mPairedDevicesList.add(bt.getName());
	...
	{
		return new ArrayList<>(mPairedDevices);
	}
